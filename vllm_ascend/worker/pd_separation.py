...
class PDDualStreamGraphManager:
    ...
    def capture(
        ...
        # token counts.
        #
        # NOTE: ``acl_graph.reset_graph_params`` was removed in the 0.23
        # release.  The parent runner already called ``set_graph_params``
        # during ``initialize_attn_backend``, so clear the module globals here
        # before registering the two static P/D sizes.
        acl_graph._graph_params = None
        acl_graph._draft_graph_params = None
        acl_graph.set_graph_params(
            sorted({self.prefill_ctx.max_tokens, self.decode_ctx.max_tokens})
        )
